...
exports.getStores = async (req, res) => {
  // 1. Query the database for a list of all stores TEST
...
exports.getHearts = async (req, res) => {
  ...
  const hearts = req.user.hearts.map(obj => obj.toString());
  const stores = await Store.find({});
  const heartedStores = stores.reduce((storeArray, nextStore) => {
    if (hearts.includes(nextStore._id.toString())) {
      storeArray.push(nextStore);
    }
    return storeArray;
  }, []);
  res.render('hearted', { title: 'Hearted', heartedStores });
  * End my way */
